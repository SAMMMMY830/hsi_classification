...
def main(args):
    ...
    features = feature_reader(orig_features)
    # 对聚类器做一个初始化，初始化数据
    ...
    try:
        ...
        G_adj = edge_construction(args, node_index[0], orig_label, features)
        cprint("building base-graph from adj",color="green")
        base_graph = build_graph(graph_path, G_adj, node_index[0])
        print("saving to edge_list.csv……")
        # nx.write_edgelist(base_graph, graph_path, data=False)
    print("saving feature and labels to files")
    save_feature_gt(g_feature[0],node_index[0],gt_index[0], CM.node_index_rl[0])
    init_dist_matrix(args, base_graph, CM.node_index_rl[0])   # 计算距离矩阵
