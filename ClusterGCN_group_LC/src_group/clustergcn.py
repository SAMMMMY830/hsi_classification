...
from src_group.deep_models3 import DeeperGCN
from utils import *
...
class ClusterGCNTrainer(object):
    ...
    def __init__(self, args, CM):
        ...
        self.index = self.CM.index  # 所有节点
        self.gt = self.CM.gt  # 所有节点标签
        self.node_index_rl = self.CM.node_index_rl
        self.features = self.CM.features[self.index]  # 所有节点特征 10249
        self.g_label = self.CM.ground_truth
    ...
    def create_model(self):
        ...
        # self.model = StackedGCN(self.args, self.CM.feature_count, self.CM.class_count)
    ...
    def do_forward_pass(self, cluster, Cost_emb, Cost_div):
        ...
        x_group, x_n, entropy_loss, layer = self.model(features, train_nodes, cluster, 'train', degree, layer_dyn[cluster], adj, target)
        self.layer[cluster] = layer
        loss_emb = Cost_emb(x_group[0][train_nodes], x_n[train_nodes], train_nodes)  # InfoGraphBDLoss 空间内损失
        loss_div = Cost_div(x_group[0][train_nodes], x_group[0][train_nodes])  # DivBD 空间间损失
        ...
        average_loss = entropy_loss*0.5 + contrastive_loss*0.5
    ...
    def do_prediction(self, cluster, layer):
        ...
        prediction = self.model(features, test_nodes, cluster, "test", degree, layer, adj, target)
    ...
    def train(self):
        ...
        Cost_emb, to_optim = losses.select_loss(self.args.loss_emb, self.args, self.device,
                                                to_optim)  # IG_binomial_deviance = InfoGraphBDLoss
        ...
        for epoch in epochs:
            ...
            for cluster in self.CM.clusters:
                ...
                loss_sum, node_count, layer = self.do_forward_pass(cluster,Cost_emb, Cost_div)
                loss_sum.backward()
                self.optimizer.step()
                loss_all += loss_sum * node_count
                # average_loss = self.update_average_loss(loss_sum, node_count)
            # print("layer",self.layer)
            average_loss = loss_all / len(self.CM.index)
            ...
            # if average_loss < 0.1:
            #     break
        self.best_layer =np.floor(self.best_layer / self.args.epochs)
        print(self.best_layer)
        return average_loss
        # fig, ax = plt.subplots()
