...
def label_reader(path):
    ...
    node_rls = data.iloc[:, 0].tolist()
    node_ids = data.iloc[:, 1].tolist()
    labels = data.iloc[:, 2].tolist()
    label_dict = {int(row[1]): row[2:].tolist() for row in data.values}
    # print(label_dict)
    return label_dict, node_rls, node_ids, labels
...
def feature_reader(path):
    ...
    g_features = features.iloc[:, 2:].values
    node_id_feat = {int(row[1]): row[2:] for row in features.itertuples(index=False, name=None)}
    node_id_rl = {int(row[1]): int(row[0]) for row in features.itertuples(index=False, name=None)}
    node_rl_id = {int(row[0]): int(row[1]) for row in features.itertuples(index=False, name=None)}
