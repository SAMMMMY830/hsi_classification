...
from src.GCNlayer import GraphConvSparse
from utils import *
...
class PositionEncoding(nn.Module):
    def __init__(self, args, graph):
        ...
        self.pca = self.args.pca
        if self.pca:
            self.embedding = nn.Linear(self.args.pca_dim, self.args.position_dim)
            self.pca_dim = self.args.pca_dim
            self.pca = True
        else:
            self.embedding = nn.Linear(self.node_cnt + 1, self.args.position_dim)

        self.init_dist_matrix()


    def init_dist_matrix(self):
        self.sub_dist_matrix = []
        matrix_path = os.path.join('/home/wjx/Cluster_Group/input/', self.data_name)
        try:
            self.dist_matrix = np.load(os.path.join(matrix_path,'dist_matrix.npy'))
            # self.sub_dist_matrix[cluster] = self.dist_matrix[sg_nodes]
            ...
            公式（2）
            '''
            self.dist_matrix /= np.nanmax(self.dist_matrix,axis=0, keepdims=True) # np.nanmax——最大值计算。对dist_matrix中每个元素进行归一化处理，让所有的距离值都缩放到 [-1, 1]
    ...
    def forward(self, nodes):
        ...
        pe = self.pca_matrix[nodes_flat + 1].float().to(self.device)
...
class LSTM_pooling(nn.Module):
    def __init__(self, args):
        ...
        self.args = args
        self.lstm = nn.LSTM(input_size=self.args.position_dim,
                               hidden_size=int(self.args.hid_dim/2),
                               num_layers=2, batch_first=True, dropout=0.5, bidirectional=True)
        self.base_gcn = GraphConvSparse(args.hid_dim, args.hid_dim)
        self.dropout = nn.Dropout(p=0.3)  # hidden_last

    # def forward(self, x, adj):
    #     gcn_out = self.base_gcn(x, adj)
    #     lstm_out, (hidden, c_n) = self.lstm(gcn_out.unsqueeze(0))
    #     l = x.shape[1]  # 子图长度
    #     if self.args.aggregator == 'mean':
    #         lstm_out = torch.sum(lstm_out, dim=1) / l
    #     elif self.args.aggregator == 'last':
    #         lstm_out = lstm_out[:, -1, :]
    #     else:
    ...
    def forward(self, x, node_num):
        # gcn_out = self.base_gcn(x, adj)
        lstm_out, (hidden, c_n) = self.lstm(x.unsqueeze(0))
        l = node_num  # 子图长度
        if self.args.aggregator == 'mean':
            lstm_out = torch.sum(lstm_out, dim=1) / l
        elif self.args.aggregator == 'last':
...
class PClassifier(nn.Module):
    def __init__(self, args, num_classes: int):
        ...
        self.linear_struc = nn.Sequential(
            nn.ReLU(),
            nn.Dropout(),
            nn.Linear(args.hid_dim, num_classes)
        )
        ...
        # self.linear_spec = nn.Sequential(
        #     nn.Dropout(),
        #     nn.Linear(args.hid_dim, args.hid_dim),
        #     nn.ReLU(),
        #     nn.Dropout(),
    ...
    def forward(self, average_embedding, structure_embedding):
        result = self.linear_node(average_embedding)
        result = torch.add(self.linear_struc(structure_embedding),result)
        return result

    # def forward(self, node_embedding, structure_embedding):
    #     result = self.linear_node(node_embedding)
    #     result = torch.add(result, self.linear_struc(structure_embedding))
    #     # result += self.linear_spec(spec_embedding)
    #     return result

class LSTM_neigbor(nn.Module):
    def __init__(self, args, num_nodes, output_dim):
        super(LSTM_neigbor, self).__init__()
        self.args = args
        self.embedding_layer = nn.Embedding(num_nodes, self.args.hid_dim)
        self.pooling = nn.LSTM(input_size=self.args.position_dim,
                               hidden_size=int(self.args.hid_dim/2),
                               num_layers=2, batch_first=True, dropout = 0.5, bidirectional=True)
        self.fc = nn.Linear(args.hid_dim * 2, output_dim)
        self.dropout = nn.Dropout()  # hidden_last

    def forward(self, nodes, adj):
        subgraph_nodes, subgraph_edge_index, mapping, _ = k_hop_subgraph(nodes, 1, adj,relabel_nodes=True)
        x = self.embedding_layer(subgraph_nodes)
        x = x.unsqueeze(0)
        lstm_out, (hidden, c_n) = self.pooling(x)
        # l = z.shape[1]  # 子图长度
        if self.args.aggregator == 'mean':
