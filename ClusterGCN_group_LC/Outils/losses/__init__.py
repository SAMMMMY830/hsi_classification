...
def select_loss(loss_type, args, device, to_optim=[]):
    if loss_type == 'margin':
        ...
        loss = binomial_deviance.BinomialDevianceLoss(args,device)
        return loss, to_optim
    elif loss_type == 'IG_binomial_deviance':
        loss = binomial_deviance.InfoGraphBDLoss(args, device)
        return loss, to_optim
