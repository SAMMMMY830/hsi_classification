...
class InfoGraphBDLoss(torch.nn.Module):
    def __init__(self, args, device):
        ...
        self.lam_glb = args.lam_glb   # 0.01
        self.add_global_group = args.add_global_group # store_true
        self.device = device
        self.top_k = args.top_k # 11
        self.k = args.num_group[0]

    def forward(self, z_g, z_n, batch):  # loss_emb = Cost_emb(x_group[0], x_n, index_all)
        #normalize(input: Tensor, p: float = 2.0, dim: int = 1, eps: float = 1e-12,
                      # out: Optional[Tensor] = None) -> Tensor:
        z_g = F.normalize(z_g, p=2, dim=-1)    # 对x_groupL2归一化
        z_n = F.normalize(z_n, p=2, dim=-1)    # 对x_n L2归一化
        z_n = torch.unsqueeze(z_n, dim=1)
        z_n = z_n.repeat(1, self.k, 1)

        _, k, _ = z_g.size() # 获取分组数
        ng = k - 1 if self.add_global_group else k  # self.add_global_group = false时 ng = k
        loss_local = 0.0    # 初始化局部损失

        for i in range(ng):
            loss_local += self.bd_loss(z_g[:, i, :], z_n[:, i, :], batch, top_k=self.top_k)  # 对每一组特征求局部损失 = neg_loss+pos_loss

        loss = loss_local / torch.tensor(ng, dtype=torch.float32)

        return loss

    def bd_loss(self, z, z_n, batch, top_k, param=(2.0, 0.5, 25)):  # z_g[:, i, :], z_n, batch, top_k=self.top_k
        """
        Args:
            x: num_graph * fea_dim
            y: num_graph * 1
    ...
    def neg_loss_graph(self, z, param=(2.0, 0.5, 25)):
        ...
        N = z.shape[0]
        y = torch.arange(N).to(self.device)

        a = torch.tensor(1e-5, dtype=data_type).to(self.device)  # 正则化项
        # neg_loss
        ...
        neg_loss = torch.log(torch.exp(neg_act) + torch.tensor(1.0, dtype=data_type).to(self.device)) * norm_neg_mask   # 对数损失函数，最小化正样本间的距离同时最大化负样本间的距离
        return torch.sum(neg_loss)
