...
def _split_with_min_per_class3(X, y, train_num, list):
    """
    ...
    train_idx = []
    test_idx = []
    class_sample_counts = np.zeros(len(list))  # 用于存储每个类别选择的样本数量
    ...
    for c in classes:
        # 获取当前类别下所有样本的索引
        list[int(c)] = class_counts[i]
        if class_counts[i] > count:
            count = class_counts[i]
            max_index = c
        idx_c = np.where(y == c)[0]

        # 将当前类别下的样本按索引排序，以确保随机选择时的一致性
        np.random.shuffle(idx_c)

        # 将当前类别下的样本数量计算为 min(train_num / len(classes), class_counts[c])
        num_samples = int(min(train_num // len(classes), list[int(c)]))

        # 将前 num_samples 个样本添加到训练集，剩余的样本添加到测试集
        train_idx.extend(idx_c[:num_samples])
        test_idx.extend(idx_c[num_samples:])

        # 存储当前类别选择的样本数量
        class_sample_counts[int(c)] = num_samples
        i = i + 1
        # 检查训练集数量是否满足 train_num
    while len(train_idx)  < train_num:
        # 从该类别中随机选择一个样本添加到训练集
        idx_max = np.where(y == max_index)[0]
        selected_idx = np.random.choice(idx_max)
        train_idx.append(selected_idx)
        test_idx.remove(selected_idx)
        # 更新该类别的样本数量
        class_sample_counts[int(max_index)] += 1
    return train_idx, test_idx, class_sample_counts[1:]


# 绝对划分，取类别数作为训练集划分标准
def _split_with_min_per_class2(X, y,train_num):
    """
    在按照百分比划分训练集时，最少每个类有一个样本作为训练集的训练对象
    ...
    for c in classes:
        ...
        # 将随机打乱后的第一个样本加入训练集
        train_idx.append(idx_c[0])

        # 将随机打乱后的其他样本加入测试集
        test_idx.extend(idx_c[1:])
        # 检查训练集大小是否满足train_num
    while len(train_idx) < train_num:
        # 从测试集中随机选择一个样本添加到训练集
        selected_idx = np.random.choice(test_idx)
        train_idx.append(selected_idx)
        test_idx.remove(selected_idx)
...
def _split_with_min_per_class(X, y, test_size,list, min_per_class=2, target_train_samples=20, random_state=None):
    ...
    for c in classes:
        ...
        # 如果当前类别样本数大于或等于最小训练样本数，则按照给定比例划分训练集和测试集
            ...
            # 计算当前类别需要保留的测试样本数
            n_test = int(np.ceil(len(idx_c) * test_size))-min_train_samples
            ...
            class_sample_counts[int(c-1)] = n_train
            class_test_counts[int(c-1)] = n_test
...
def _split_with_min_per_class1(X, y, test_size,list, min_per_class=2, target_train_samples=20, random_state=None):
    ...
    return train_data_index, test_data_index, class_sample_counts, class_test_counts
