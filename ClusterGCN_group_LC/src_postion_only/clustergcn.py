...
from utils import edge_construction
...
class ClusterGCNTrainer(object):
    ...
    def __init__(self, args, CM, PE):
        ...
        self.sg_targets = self.CM.sg_gt  # 子图标签列表
    ...
    def create_model(self):
        ...
        for i in self.clusters:
            self.adj[i] = self.edge_construction(self.train_node[i], self.sg_nodes[i], self.train_node[i])
            self.degree[i] = self.adj[i].sum(0)
            # self.sg[i] = self.build_graph(self.adj[i])
    ...
    def do_forward_pass(self, cluster, Cost_emb, Cost_div):
        ...
        target = self.CM.sg_gt[cluster].to(self.device).squeeze()
        degree = self.degree[cluster].to(self.device)
        adj = self.adj[cluster].to(self.device)
        # 将子图的边和特征传入，得到预测值，并计算损失
        layer_dyn = np.zeros(self.num_graph)
        entropy_loss = self.model(sg_nodes, features, train_nodes, cluster, 'train', degree, layer_dyn[cluster], adj, target)
        ...
        return entropy_loss, node_count
    ...
    def do_prediction(self, cluster):
        ...
        prediction = self.model(sg_nodes, features, test_nodes, cluster, 'test', degree, 3, adj, target)
    ...
    def train(self):
        ...
        for epoch in epochs:
            ...
            average_loss = loss_all / len(self.CM.nodes)
